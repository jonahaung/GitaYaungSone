...
class ChordsInputView: UIView {
    ...
    private let keys = Chord.Key.allCases.map{ $0.rawValue }
    private let suffixs = Chord.Suffix.allCases.map{ $0.rawValue }
...
extension ChordsInputView {
    ...
    private func configureDataSource() {
        let cellRegistration = UICollectionView.CellRegistration<ChordInputCell, String> { (cell, indexPath, identifier) in
